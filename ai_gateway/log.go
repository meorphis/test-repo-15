...
// List Gateway Logs
func (r *LogService) List(ctx context.Context, accountID string, id string, query LogListParams, opts ...option.RequestOption) (res *pagination.V4PagePaginationArray[LogListResponse], err error) {
	...
	path := fmt.Sprintf("accounts/%s/ai-gateway/gateways/%s/logs", accountID, id)
...
// List Gateway Logs
func (r *LogService) ListAutoPaging(ctx context.Context, accountID string, id string, query LogListParams, opts ...option.RequestOption) *pagination.V4PagePaginationArrayAutoPager[LogListResponse] {
	return pagination.NewV4PagePaginationArrayAutoPager(r.List(ctx, accountID, id, query, opts...))
}
